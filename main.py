'''
To compile code, run python main.py --corpus=wsj-clean.txt --pos=piper.txt
You just need to pass the corpus text file to corpus and test text file to pos 
'''
import argparse
...
def oracle_parsing(tsdata,left_arc,right_arc,confusion_arc):
    ...
    print stack_test,buffer_test,"SHIFT"
    ...
    while (len(stack_test) > 0):
        ...
        if (len(stack_test) == 1 and len(buffer_test) == 0):
            print stack_test,buffer_test, "ROOT -->", stack_test[0]
            stack_test.pop(0)
        ...
        if len(stack_test) > 1:
            ...
            if second_ith_tag[0] == "V" and (top_jth_tag[0] == "." or top_jth_tag[0] == "R"):
                print stack_test,buffer_test, "Right Arc:",stack_test[-2]," -->",stack_test[-1]
                stack_test.pop(-1)
                
            elif second_ith_tag[0] ==  "I" and top_jth_tag[0] == ".":
                print stack_test,buffer_test,"SWAP"
            ...
            #confusion arc
                ...
                if confusion_arc[second_ith_tag][top_jth_tag].index(max(confusion_arc[second_ith_tag][top_jth_tag])) == 1:
                    ...
                    print stack_test,buffer_test, "Left Arc:",stack_test[-2]," <--",stack_test[-1]
                    stack_test.pop(-2)
            #left arc
...
print "\n"
print "\tNumber of confusing arcs = ",count
